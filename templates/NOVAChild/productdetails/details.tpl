{block name='productdetails-details'}
        ...
        {literal}
            <script>
                $(document).ready(function() {
                    $('#putIntoBasket').modal('show');
                });
            </script>
        {/literal}
            <div class="modal fade" id="putIntoBasket" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{lang key='basketAdded' section='messages'}</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            {include file='productdetails/pushed_success.tpl' card=false}
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        {if $Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen !== 'N' && isset($Artikel->cHersteller)}
                            {block name='productdetails-details-product-info-manufacturer'}
                                <li class="product-manufacturer" itemprop="brand" itemscope itemtype="https://schema.org/Brand">
                                    <strong>{lang key='manufacturerSingle' section='productOverview'}:</strong>
    ...
    {block name='productdetails-details-content-not-quickview'}
        {if isset($Xselling->Standard->XSellGruppen) && count($Xselling->Standard->XSellGruppen) > 0}
            ...
            {/block}
            {/col}
            {col cols=12 lg=5 class='mt-3'}
            {foreach $Xselling->Standard->XSellGruppen as $Gruppe}
                {include file='snippets/product_slider.tpl' tplscope='half' class='x-supplies' id='slider-xsell-group-'|cat:$Gruppe@iteration productlist=$Gruppe->Artikel title=$Gruppe->Name}
            {/foreach}
            {/col}
            {/row}
            {/container}
