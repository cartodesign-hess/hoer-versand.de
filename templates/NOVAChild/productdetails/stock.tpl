                ...
                {if !isset($availability)}
                    {block name='productdetails-stock-estimated-delivery'}
                        {if $Artikel->cEstimatedDelivery}
                            ...
                            <li>{* carto: change popover to tooltip *}
                                <div class="estimated-delivery"
                                        {if isset($oSpezialseiten_arr[$smarty.const.LINKTYP_VERSAND])}
                                    data-toggle="tooltip"
                                    data-placement="top"
                                    data-html="true"
                                    title="{if $selectedCountry !== null}{lang key='shippingInformation' section='productDetails' assign=silv}{sprintf($silv, $selectedCountry->getName(), $oSpezialseiten_arr[$smarty.const.LINKTYP_VERSAND]->getURL(), $oSpezialseiten_arr[$smarty.const.LINKTYP_VERSAND]->getURL())}{/if}"
                                        {/if}>
