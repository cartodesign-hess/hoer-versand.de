        ...
        <div class="price_wrapper">
            ...
            {else}
                {block name='productdetails-price-label'}
                    ...
                    {/if}
                {/block}
                <div class="price {if $priceLarge|default:false}h2 mb-0{else}productbox-price{/if} {if isset($Artikel->Preise->Sonderpreis_aktiv) && $Artikel->Preise->Sonderpreis_aktiv} special-price{/if}">
                ...
                {if $tplscope === 'detail'}
                    ...
                    {if $Einstellungen.artikeldetails.artikel_weitere_artikel_hersteller_anzeigen === 'Y'}
                        ...
                        {/if}
                        itemprop="url" style="float: right; padding-bottom: 10px;">
                    {/if}
                    {if ($Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen === 'B'
                    || $Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen === 'BT')
                    && !empty($Artikel->cHerstellerBildURLNormal)}
                        {image lazy=true
                        webp=true
                        src=$Artikel->cHerstellerBildURLNormal
                        alt=$Artikel->cHersteller|escape:'html'
                        }
                        <meta itemprop="image" content="{$Artikel->cHerstellerBildURLNormal}">
                    {/if}
                    {if $Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen !== 'B'}
