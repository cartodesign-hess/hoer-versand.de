{block name='productlist-item-list'}
    ...
    <div id="{$idPrefix|default:''}result-wrapper_buy_form_{$Artikel->kArtikel}" data-wrapper="true"
         class="productbox productbox-row productbox-show-variations {if isset($listStyle) && $listStyle === 'list'} active{/if}">
        <div class="productbox-inner">
            {block name='productlist-item-list-productbox-inner'}
            {row}
                {col cols=12 sm=4 md=3}
                    ...
                    {/block}
                {/col}
                {col cols=12 sm=8 md=9}
                    {row}
                        {col cols=12 md=8 lg=12 xl=8}
                        ...
                        {block name='productlist-item-list-buy-form'}
                            ...
                            {col cols=12 class='productbox-variations'}
                                ...
                                {/if}
                            {/col}
                            {col cols=12 class='productbox-options' itemprop='offers' itemscope=true itemtype='https://schema.org/Offer'}
                                    ...
                                    <div class="form-row productbox-actions item-list-basket-details mb-5">
                                            ...
                                            {else}
                                                ...
                                                {block name='productlist-item-list-basket-details-add-to-cart'}
                                                    ...
                                                    aria=["label"=>{lang key='addToCart'}]}
                                                        <span style="margin-right:0.25rem">{lang key='addToCart'}</span> <i class="fas fa-shopping-cart"></i>
                                                    {/button}
                                                {/block}
                                                {/col}
                                            {/if}
                                        {else}
                                            ...
                                            {block name='productlist-item-list-basket-details-details'}
                                                {link class="btn btn-primary btn-block" role="button" href=$Artikel->cURLFull}
                                                {lang key='details'}
                                                {/link}
                                            {/block}
                                            {/col}
