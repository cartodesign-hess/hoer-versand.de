{block name='layout-header'}
    ...
    {block name='layout-header-header'}
        <div class="container-fluid text-center header-banner-row">{* Carto [ch]: Add note *}
            {getBoxesByPosition position='top' assign='headerBoxes'}
            {if isset($headerBoxes) && count($headerBoxes) > 0}
                {foreach $headerBoxes as $box}
        ...
        {block name='layout-header-branding-top-bar'}
            <div id="header-top-bar" class="topbar-wrapper {if $Einstellungen.template.header.menu_single_row === 'Y'}full-width-mega{/if} {if $Einstellungen.template.header.header_full_width === 'Y'}is-fullwidth{/if} d-lg-flex">
                <div class="container {if $headerWidth === 'B'}container{else}container-fluid {if $headerWidth === 'N'}container-fluid-xl{/if}{/if} d-lg-flex justify-content-between flex-row-reverse">
        ...
        <header class="d-print-none {*if !$isMobile || $Einstellungen.template.theme.mobile_search_type !== 'fixed'}sticky-top{/if*} fixed-navbar" id="jtl-nav-wrapper">
            {block name='layout-header-container-inner'}
                <div class="container {if $Einstellungen.template.megamenu.header_full_width === 'N'}container-fluid-xl{/if}">
                    {block name='layout-header-category-nav'}
                        {navbar toggleable=true fill=true type="expand-lg" class="justify-content-between align-items-center-util"}
                            <div class="toggler-logo-wrapper">
                                {block name='layout-header-navbar-toggle'}{/block}{* Carto: Remove to header_nav_icons.tpl *}
                                ...
                                {block name='layout-header-logo'}
                                    <div id="logo" class="logo-wrapper" itemprop="publisher" itemscope itemtype="https://schema.org/Organization">
                                        ...
                                        {if isset($ShopLogoURL)}
                                            {image width=180 height=60 src=$ShopLogoURL
                        ...
                        {else}
                                ...
                                <div class="search-wrapper">
                                    {form action="{get_static_route id='index.php'}" method='get'}
                                        <div class="form-icon">
                                            ...
                                            {button type="submit" name='search' variant="secondary" aria=["label"=>{lang key='search'}]}<span class="fa-light fa-magnifying-glass"></span>{* Carto: Change icon *}{/button}
                                            {/inputgroupaddon}
                                            {/inputgroup}
                                        </div>
                                    {/form}
                            ...
                            {block name='layout-header-branding-shop-nav'}
                                ...
                                {block name='layout-header-branding-shop-nav-include-language-dropdown'}{/block}{* Carto: Remove mobile language dropdown *}
                ...
                {if $nSeitenTyp !== $smarty.const.PAGE_BESTELLVORGANG}
                    <div class="container-fluid {if $Einstellungen.template.megamenu.header_full_width === 'N'}container-fluid-xl{/if} second-row">
                        {navbar toggleable=true fill=true type="expand-lg" class="justify-content-end align-items-center-util"}
                            {*categories*}
                            {block name='layout-header-include-categories-mega'}
                                <div id="mainNavigation" class="collapse navbar-collapse">{* carto: remove class 'nav-scrollbar' *}
                                    ...
                                    {block name='layout-header-include-include-categories-body'}
                                        <div class="nav-mobile-body d-lg-none">
                                            {navbarnav class="nav-scrollbar-inner mr-auto justify-content-center"}
                                        ...
                                        <div class="d-none d-lg-block m-auto">
                                            {include file='snippets/categories_menu_nested.tpl'}
                                        </div>
                                    {/block}
                                </div>
                            {/block}
        ...
        {*include file='snippets/help_button.tpl'*}
    ...
    {block name='layout-header-content-all-starttags'}
        {block name='layout-header-content-wrapper-starttag'}
            <div id="content-wrapper"
                 class="container {if ($Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive) || $smarty.const.PAGE_ARTIKELLISTE === $nSeitenTyp}has-left-sidebar container-fluid container-fluid-xl{/if}
