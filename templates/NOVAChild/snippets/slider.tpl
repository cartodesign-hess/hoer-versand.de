{block name='snippets-slider'}
    {if isset($oSlider) && count($oSlider->getSlides()) > 0}
        {container fluid=$isFluid class="{if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive}container-plus-sidebar{/if}"}
            ...
            <div class="desktop d-none d-lg-block slider-wrapper theme-{$oSlider->getTheme()}{if $oSlider->getControlNav()} control-nav{/if}{if $oSlider->getDirectionNav()} direction-nav{/if}{if $oSlider->getThumbnail()} thumbnail-nav{/if}">
            ...
            {* Carto: Mobile version *}
            <div class="mobile d-lg-none slider-wrapper theme-{$oSlider->getTheme()}{if $oSlider->getControlNav()} control-nav{/if}{if $oSlider->getDirectionNav()} direction-nav{/if}{if $oSlider->getThumbnail()} thumbnail-nav{/if}">
                <div id="slider-{$oSlider->getID()}-mobile" class="nivoSlider">

                    {foreach $oSlider->getSlides() as $oSlide}
                    {assign var=slideTitle value=$oSlide->getTitle()}
                    {if !empty($oSlide->getText())}
                        {assign var=slideTitle value="#slide_caption_{$oSlide->getID()}_mobile"}
                    {/if}
                    {if !empty($oSlide->getLink())}
                    <a href="{$oSlide->getLink()}"{if !empty($oSlide->getText())} title="{$oSlide->getText()|strip_tags}"{/if} class="slide">
                        {else}
                        <div class="slide">
                            {/if}
                                {image alt=$oSlide->getTitle()
                                title=$slideTitle
                                src=$oSlide->getAbsoluteImage()|replace:'.jpg':'-mobil.jpg'
                                data=["thumb" => "{if !empty($oSlide->getAbsoluteThumbnail())}{$oSlide->getAbsoluteThumbnail()}{/if}"]}
                            {if !empty($oSlide->getLink())}
                    </a>
                    {else}
                ...
                {foreach $oSlider->getSlides() as $oSlide}
                    {if !empty($oSlide->getText())}
                        <div id="slide_caption_{$oSlide->getID()}_mobile" class="htmlcaption d-none">
                            {if isset($oSlide->getTitle())}
                                    <strong class="title">{$oSlide->getTitle()}</strong>
                            {/if}
                                <p class="desc">{$oSlide->getText()}</p>
            ...
            {inline_script}<script>
                {if $oSlider->getUseKB() === false}
                    $(function () {
                        ...
                        {* Carto: Mobile version *}
                        var slider_m = $('#slider-{$oSlider->getID()}-mobile');
                        $('a.slide').on('click', function () {
                            if (!this.href.match(new RegExp('^' + location.protocol + '\\/\\/' + location.host))) {
                                this.target = '_blank';
                            }
                ...
                {else}
                    ...
                    $(function () {
                        ...
                        {* Carto: Mobile version *}
                        var slider_m = $('#slider-{$oSlider->getID()}');
                        var endSlide_m=$('.nivoSlider img').length-1;
                        $('a.slide').click(function() {
                            if (!this.href.match(new RegExp('^'+location.protocol+'\\/\\/'+location.host))) {
                                this.target = '_blank';
                            }
