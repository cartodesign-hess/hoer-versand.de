        ...
        {block name='checkout-customer-shipping-address-city-wrap'}
            {col cols=12 md=4}
                {block name='checkout-customer-shipping-address-zip'}
                    {formgroup
                        class="typeahead-required{if !empty($fehlendeAngaben.plz)} has-error{/if} postcode-wrapper"
                        label="{lang key='plz' section='account data'}"
                        label-for="{$prefix}-{$name}-postcode"
                    }
                        {if isset($fehlendeAngaben.plz)}
                            <div class="form-error-msg" aria-live="assertive" role="alert" aria-atomic="true"><i class="fa fa-exclamation-triangle"></i>
                        ...
                        {input
                            ...
                            class="postcode_input typeahead bg-typeahead-fix"
                            placeholder=" "
            ...
            {col cols=12 md=8}
                {block name='checkout-customer-shipping-address-city'}
                    {formgroup
                        class="typeahead-required{if !empty($fehlendeAngaben.ort)} has-error{/if} city-wrapper"
                ...
                {col cols=12 md=12}
                    {block name='checkout-customer-shipping-address-is-default'}
                        ...
                        checked=isset($Lieferadresse->nIstStandardLieferadresse) && $Lieferadresse->nIstStandardLieferadresse === 1
                            ...
                            {lang key='checkoutSaveShippingAddressAsDefault' section='account data'}
                        {/checkbox}
                    {/block}
                {/col}
            {/if}
